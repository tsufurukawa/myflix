...
bootcamp Tealeaf Academy (http://www.gotealeaf.com). The UI mockup was provided to us by the 
...
You can either sign in with "admin@example.com" and "password" or register for an account with
credit card number "4242424242424242" and Security Code "123" to log-in to the application. 
(Note: the former method will sign you in as an Administrator.) 
...
* Accepting payments with custom form using Stripe.js.
* Responding to webhooks to handle successful and failed charge events.
...
Myflix has two different types of users, or "actors" - a regular user and an admin. An admin can interact with
...
The Paratrooper gem was used to simply the deployment process with quick and concise rake tasks. 